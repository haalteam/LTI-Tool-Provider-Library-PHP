    ...
    public function saveToolConsumer($consumer)
    {
        ...
        if (empty($id)) {
            $sql = sprintf("INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::CONSUMER_TABLE_NAME . ' (consumer_key256, consumer_key, name, ' .
                           ...
                           $this->escape($key256), $this->escape($key), $this->escape($consumer->name),
                           $this->escape($consumer->secret), $this->escape($consumer->ltiVersion),
                           $this->escape($consumer->consumerName), $this->escape($consumer->consumerVersion), $this->escape($consumer->consumerGuid),
                           $this->escape($profile), $this->escape($consumer->toolProxy), $this->escape($settingsValue),
                           $protected, $enabled, $this->escape($from), $this->escape($until), $this->escape($last),
            ...
            $sql = sprintf("UPDATE {$this->dbTableNamePrefix}" . DataConnector::CONSUMER_TABLE_NAME . ' SET ' .
                           ...
                           'WHERE consumer_pk = %d',
                           $this->escape($key256), $this->escape($key),
                           $this->escape($consumer->name),
                           $this->escape($consumer->secret), $this->escape($consumer->ltiVersion),
                           $this->escape($consumer->consumerName), $this->escape($consumer->consumerVersion), $this->escape($consumer->consumerGuid),
                           $this->escape($profile), $this->escape($consumer->toolProxy), $this->escape($settingsValue),
                           $protected, $enabled,
                           $this->escape($from), $this->escape($until), $this->escape($last),
                           $this->escape($now), $consumer->getRecordId());
        }
    ...
    public function loadContext($context)
    {
        ...
        if (!is_null($context->getRecordId())) {
            ...
            $sql = sprintf('SELECT context_pk, consumer_pk, lti_context_id, settings, created, updated ' .
                           ...
                           $context->getConsumer()->getRecordId(), $this->escape($context->ltiContextId));
        }
    ...
    public function saveContext($context)
    {
        ...
        if (empty($id)) {
            $sql = sprintf("INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::CONTEXT_TABLE_NAME . ' (consumer_pk, lti_context_id, ' .
                           ...
                           'VALUES (%d, %s, %s, %s, %s)',
               $consumer_pk, $this->escape($context->ltiContextId),
               $this->escape($settingsValue),
               $this->escape($now), $this->escape($now));
        } else {
            $sql = sprintf("UPDATE {$this->dbTableNamePrefix}" . DataConnector::CONTEXT_TABLE_NAME . ' SET ' .
               ...
               $this->escape($context->ltiContextId), $this->escape($settingsValue),
    ...
    public function loadResourceLink($resourceLink)
    {
        ...
        if (!is_null($resourceLink->getRecordId())) {
            ...
            $sql = sprintf('SELECT resource_link_pk, context_pk, consumer_pk, lti_resource_link_id, settings, primary_resource_link_pk, share_approved, created, updated ' .
                           ...
                           $resourceLink->getContext()->getRecordId(), $this->escape($resourceLink->getId()));
        } else {
            $sql = sprintf('SELECT r.resource_link_pk, r.context_pk, r.consumer_pk, r.lti_resource_link_id, r.settings, r.primary_resource_link_pk, r.share_approved, r.created, r.updated ' .
                           ...
                           $resourceLink->getConsumer()->getRecordId(), $resourceLink->getConsumer()->getRecordId(), $this->escape($resourceLink->getId()));
        }
    ...
    public function saveResourceLink($resourceLink) {
        ...
        if (empty($id)) {
            $sql = sprintf("INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' (consumer_pk, context_pk, ' .
                           ...
                           $consumerId, $contextId, $this->escape($resourceLink->getId()),
                           $this->escape($settingsValue),
                           $primaryResourceLinkId, $approved, $this->escape($now), $this->escape($now));
            ...
            $sql = sprintf("UPDATE {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' SET ' .
                           ...
                           $consumerId, $this->escape($resourceLink->getId()),
                           $this->escape($settingsValue), $primaryResourceLinkId, $approved, $this->escape($now),
                           $contextId, $id);
        } else {
            $sql = sprintf("UPDATE {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' SET ' .
                           ...
                           $contextId, $this->escape($resourceLink->getId()),
                           $this->escape($settingsValue), $primaryResourceLinkId, $approved, $this->escape($now),
                           $consumerId, $id);
        }
        $ok = mysqli_query($this->db, $sql);
 ...
 */
    public function saveResourceLinkShareKey($shareKey)
    {
        ...
        $sql = sprintf("INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_SHARE_KEY_TABLE_NAME . ' ' .
                       ...
                       $this->escape($shareKey->getId()), $shareKey->resourceLinkId);
        $ok = mysqli_query($this->db, $sql);
    ...
    public function loadUser($user)
    {
        ...
        if (!is_null($user->getRecordId())) {
            ...
            $sql = sprintf('SELECT user_pk, resource_link_pk, lti_user_id, lti_result_sourcedid, created, updated ' .
                           ...
                           $this->escape($user->getId(ToolProvider\ToolProvider::ID_SCOPE_ID_ONLY)));
        }
        $rsUser = mysqli_query($this->db, $sql);
        if ($rsUser) {
    ...
    public function saveUser($user)
    {
        ...
        if (is_null($user->created)) {
            $sql = sprintf("INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::USER_RESULT_TABLE_NAME . ' (resource_link_pk, ' .
                           ...
                           'VALUES (%d, %s, %s, %s, %s)',
                           $user->getResourceLink()->getRecordId(),
                           $this->escape($user->getId(ToolProvider\ToolProvider::ID_SCOPE_ID_ONLY)), $this->escape($user->ltiResultSourcedId),
                           $this->escape($now), $this->escape($now));
        } else {
            $sql = sprintf("UPDATE {$this->dbTableNamePrefix}" . DataConnector::USER_RESULT_TABLE_NAME . ' ' .
                           'SET lti_result_sourcedid = %s, updated = %s ' .
                           'WHERE (user_pk = %d)',
                           $this->escape($user->ltiResultSourcedId),
                           $this->escape($now),
                           $user->getRecordId());
        }
        $ok = mysqli_query($this->db, $sql);
 ...
 */
    function escape($value, $addQuotes = true)
    {
